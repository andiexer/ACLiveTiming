...
public sealed class TrackMapService
{
    ...
    private readonly ConcurrentDictionary<string, TrackMapConfig?> _cache = new(StringComparer.OrdinalIgnoreCase);
    ...
    private TrackMapConfig? LoadConfig(string slug)
    {
        ...
        float Get(string key, float fallback) =>
            dict.TryGetValue(key, out var v) && float.TryParse(v, CultureInfo.InvariantCulture, out var f) ? f : fallback;
